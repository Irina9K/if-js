...
function combinedParam(arr1, arr2) {
  const result = arr1.map((item, i) => {
    if (item.role.toLowerCase() === 'admin') {
      delete item.role;
      const role = Object.assign(item, { role: { id: arr2[0].id, name: arr2[0].name } });
      return role;
    }

    if (item.role.toLowerCase() === 'teacher') {
      delete item.role;
      const role = Object.assign(item, { role: { id: arr2[1].id, name: arr2[1].name } });
      const studentsId = Object.assign(item, { studentIds: arr1[i].students });
      delete item.students;
      return Object.assign(role, studentsId);
    }

    if (item.role.toLowerCase() === 'student') {
      delete item.role;
      const role = Object.assign(item, { role: { id: arr2[2].id, name: arr2[2].name } });
      const teachersId = Object.assign(item, { teachersIds: arr1[i].teachers });
      delete item.teachers;
      return Object.assign(role, teachersId);
    }
...
console.log(combinedParam(users, roles));
...
const adminInstance = new Admin(users[2]);
console.log(adminInstance);

class Teacher extends User {
  get getStudents() {
    const getSt = this.usersArr.reduce((newArr, item) => {
      if (item.studentIds) {
        newArr.push(`Student {${item.studentIds}}`);
      }
...
const teacherInstance = new Teacher(users);
console.log(teacherInstance);
console.log(teacherInstance.getStudents);

class Student extends User {
  get getTeachers() {
    const getTh = this.usersArr.reduce((newArr, item) => {
      if (item.teachersIds) {
        newArr.push(`Teacher {${item.teachersIds}}`);
      }
...
const studentInstance = new Student(users);
console.log(studentInstance);
console.log(studentInstance.getTeachers);
